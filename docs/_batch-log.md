...
- **#66 — fg_ledger schema mismatch broke spawn entirely (deployed, awaiting verify)** — `D4JSP/aceb37a` deployed. Adam saw "Spawn failed Could not find the 'from_uid' [...] schema cache" toast — the smoking gun. Production `fg_ledger` is entity-based (`tx_type/from_entity/to_entity/memo/metadata`), NOT uid-based as the docs / `gamble.js` / my #61 fix all assumed. Rewrote the ledger inserts in quest-trigger.js + forum-trolls.js to use the actual schema. Updated `docs/data-model/fg-ledger.md` to reflect reality + flag the planned migration as NOT YET APPLIED. Same bug exists in `pages/api/admin/gamble.js` — file as separate follow-up.
- **#65 — Weekly spawn cap disabled for testing (no-op)** — Adam: "for now weekly spawn should be off for testing"; "good if it's working as a setting now though". Verified: `triggers.config` for `forum_troll_spawned` is `{click_range_max:10, click_range_min:1}` — `max_per_week` field is unset, so the server check `if (cfg.max_per_week)` is already a no-op. No DB change needed. The setting is correctly modular (admin-editable per #46) — flip it back via admin Triggers tab when launching: Admin → Features → Triggers → forum_troll_spawned → config → set `max_per_week` to desired integer.
- **#64 — Diagnostic toast surface on gem-click silent paths (deployed)** — `D4JSP/2c8de22` deployed. Surfaces visible toast on every silent-failure path in `handleGemClick`. Adam clicked once and got "Spawn failed Could not find the 'from_uid' ... schema cache" — exposing #66 as the actual root cause. Will keep the toast surface in place permanently as a diagnostic; no rollback.
- **#63 — Gem onClick over-blocked when no troll alive (deployed)** — `D4JSP/9e2f15b` deployed. Tightened the lock predicate to require `activeTrolls.some(isAlive)` instead of `length > 0`, both in HomeView trollActive prop and AppShell handleGemClick. Stale ghost rows no longer block. Necessary but insufficient — #66 was the real spawn blocker.
