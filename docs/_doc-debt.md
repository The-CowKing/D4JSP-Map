...
- ~~**Persistent gem-glow not actually persistent**~~ — RESOLVED by [#45 / `bb200ce`](./_batch-log.md). `/api/forum-trolls` now returns `Cache-Control: no-store`; realtime INSERT propagates to `trollActive` within ~1s.
