...
- **#55 ACTUAL fix** (2026-04-26) — After two failed CSS-wrap attempts, Adam diagnosed the real cause: the desktop "tooltip clipping" was the **TITLE TEXT visually overlapping the tooltip's space** on desktop, making the tooltip look cut off. Not internal text wrap. Spec: "PC text should be center of card any ways.. phone it should stay how it is." Fix: `className="trade-card-title-row"` on the title bar + `@media (min-width: 768px) { .trade-card-title-row { justify-content: center } }` in `HomeView.js` VIEW_CSS. Mobile stays flex-start (left-aligned next to tooltip — current good behavior). Desktop centers the title inside the right panel, away from the tooltip. The two earlier CSS-wrap attempts kept in place as defensive layer.
- **#55 follow-up fix (didn't solve)** (2026-04-26) — Adam reported desktop clipping STILL present after #55 deploy + cache clear. Two missing pieces in the first pass:
  1. The `.whtt-scroll *` rule had `overflow-wrap:anywhere` + `word-break:break-word` but NO `white-space:normal`. Those only kick in when wrapping is allowed; if any parent (e.g. nested td or styled descendant) had `white-space: nowrap` inherited, the child text stayed single-line and got clipped. Adam's screenshot showed mid-word truncation with ellipsis — the hallmark of `white-space:nowrap + text-overflow:ellipsis`, not multi-line wrapping. Added `white-space:normal !important` to the universal rule.
  2. `.whtt-container` had `overflow:visible !important` so any descendant content extending past the locked 300px width visibly bled out — `D4Tooltip.writeLayout` then measured `inner.scrollWidth > 300`, sized the wrapper too wide, and `.feed-thumb { overflow: hidden }` clipped the right edge. Changed container to `overflow:hidden !important`. The floating `.whtt-tooltip-icon` is a SIBLING of `.whtt-container` (not a descendant), so this overflow:hidden does NOT clip the icon.
  3. Username `wordBreak: 'break-word'` → `'break-all'` + explicit `whiteSpace: 'normal'`. break-all breaks at any character (not just unbreakable words) — guarantees wrap to 2 lines via WebkitLineClamp:2 even when Cinzel uppercase glyphs render slightly wider than the 60px content area on desktop.
...
9. **The desktop wrap rules** — `white-space:normal!important; overflow-wrap:anywhere!important; max-width:300px!important` on `.wowhead-tooltip[data-game="d4"]` table/th/td, AND **`white-space:normal!important; overflow-wrap:anywhere!important; word-break:break-word!important`** on `.whtt-scroll *`. Wowhead's base CSS only forces wrap on `max-width:599px`; without our global mirror, desktop tooltips overflow their locked 300px width on long flavor text. The `white-space:normal` part of the `.whtt-scroll *` rule is THE critical bit — without it, inherited nowrap from parent elements keeps text on a single line that gets ellipsis-truncated (the #55 first-pass missed this).
10. **`.wowhead-tooltip[data-game="d4"] .whtt-container { overflow: hidden !important }`.** Anything else (visible/auto) lets descendant content bleed past the 300px tooltip width, which corrupts D4Tooltip's `writeLayout` scrollWidth measurement and visibly clips the right edge inside `.feed-thumb`. The floating `.whtt-tooltip-icon` is a SIBLING of `.whtt-container`, NOT a descendant — `overflow:hidden` on container does NOT clip the icon.

## Desktop layout — title centering + DO NOT NARROW the left panel
...
### Desktop title centering (#55 real fix)

Adam's spec: "PC text should be center of card any ways.. phone it should stay how it is."

The title bar (`.trade-card-title-row` in [`../../components/HomeView.js`](../../components/HomeView.js)) is a flex row inside the right panel containing the optional NEW badge, the item name (`{t.title}`), and the action-type tag. Behavior split by viewport:

| Viewport | `.trade-card-title-row` justify-content | Layout |
|---|---|---|
| Mobile (≤767px) | default `flex-start` | Title left-aligned, next to tooltip — current working layout. |
| Desktop (≥768px) | `center` (via `@media` rule in `VIEW_CSS`) | Title centered horizontally in the right panel; visually clears the tooltip. |

**Why this fixes the "tooltip clipping on PC" symptom Adam reported:** the tooltip wasn't actually clipping its own text. The title text on desktop, left-aligned at the start of the right panel, visually overlapped the tooltip's region (because at common scale 0.3 the right panel starts at leftPanelW=94px from card-left and the title rendered immediately at +6px of padding = 100px, while the tooltip's 90px visual extent came right up to that boundary, creating a cramped/conflicting visual). Centering the title on desktop pushes it visually into the open middle of the right panel and removes the perceived overlap. Mobile, where the right panel is much narrower, never had this issue.

**DO NOT remove the `@media (min-width: 768px) { .trade-card-title-row { justify-content: center } }` rule.** It is the entire fix.

### Tooltip width lock

The tooltip itself is locked at `width: 300px` (intrinsic). After scaling, visual width = 300 × scale. Always smaller than or equal to leftPanelW. The text-wrap defensive rules (`white-space: normal !important; overflow-wrap: anywhere !important; max-width: 300px !important` on tooltip tables/cells, and `white-space: normal !important; overflow-wrap: anywhere !important; word-break: break-word !important` on `.whtt-scroll *`) ensure tooltip text always wraps within 300px regardless of cascade. These were originally added as the (mistaken) fix for #55 desktop clipping; they remain in place as a defensive layer because they're correct in their own right (Wowhead's mobile-only wrap-override leaves a desktop gap that these rules close).
...
- [`../_batch-log.md`](../_batch-log.md) #48 (diagnosis), #50 (initial fix), #54 (bottom-anchor restructure), #55 (desktop right-edge wrap — first pass + follow-up) — the work history.
